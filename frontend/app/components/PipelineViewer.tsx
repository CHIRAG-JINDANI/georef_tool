...
export default function PipelineViewer({ stepImages, result, onClose }: PipelineViewerProps) {
  ...
  // States specific to the GCP Table Analysis View
  const [selectedGcpId, setSelectedGcpId] = useState<number | null>(null)

  // --- NEW: FREE FLOW PAN & ZOOM ENGINE ---
  ...
  // Clamp function enforcing the 30% maximum out-of-frame rule
  const clampPan = (newX: number, newY: number, currentScale: number) => {
    if (!containerRef.current) return { x: newX, y: newY }
    const { clientWidth, clientHeight } = containerRef.current

    // Calculate how much the image has expanded beyond the container
    ...
    // 30% threshold rule from the container sides
  ...
  const handleZoom = (delta: number) => {
    setZoomScale(s => {
      const newScale = Math.max(1, Math.min(s + delta, 10)) // Cap zoom at 10x
      setPan(prev => clampPan(prev.x, prev.y, newScale)) // Re-clamp if zooming out
      return newScale
    })
  }
  ...
  const handleDoubleClick = (e: React.MouseEvent) => {
    e.preventDefault()
    handleZoom(0.5) // Zoom in on double click
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 48, display: 'flex', alignItems: 'center', padding: '0 20px', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', gap: 16, flexShrink: 0 }}>
        <button
          onClick={onClose}
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 10px', cursor: 'pointer', color: 'var(--text-secondary)', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, transition: 'all 0.15s ease' }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border-active)'; e.currentTarget.style.color = 'var(--accent-blue)' }}
        ...
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>pipeline inspector</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: '20%', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '20px 0', gap: 2, background: 'var(--bg-panel)' }}>
          <div style={{ padding: '0 20px 12px', fontFamily: "'JetBrains Mono', monospace", fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>pipeline steps</div>
          {STEPS.map(step => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isActive ? 'rgba(99,179,237,0.08)' : isHovered && hasImage ? 'rgba(255,255,255,0.03)' : 'transparent',
                  borderLeft: isActive ? '2px solid var(--accent-blue)' : '2px solid transparent', transition: 'all 0.15s ease',
                }}
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: isActive ? 'var(--accent-blue)' : 'var(--text-muted)', letterSpacing: '0.05em', minWidth: 16 }}>{hasImage ? '✓' : `0${num}`}</span>
                    <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, fontWeight: isActive ? 600 : 400, color: isActive ? 'var(--text-primary)' : isHovered && hasImage ? 'rgba(240,240,240,0.9)' : 'var(--text-secondary)', transition: 'color 0.15s ease' }}>{step.label}</span>
                  </div>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--text-muted)', paddingLeft: 24 }}>{step.sub}</span>
                </div>
              </div>
            )
          })}

          <div style={{ padding: '24px 20px 12px', fontFamily: "'JetBrains Mono', monospace", fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>gcp analysis</div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isActive ? 'rgba(104,211,145,0.08)' : isHovered && canClick ? 'rgba(255,255,255,0.03)' : 'transparent',
                  borderLeft: isActive ? '2px solid var(--accent-green)' : '2px solid transparent', transition: 'all 0.15s ease',
                }}
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: isActive ? 'var(--accent-green)' : 'var(--text-muted)', letterSpacing: '0.05em', minWidth: 16 }}>◆</span>
                    <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, fontWeight: isActive ? 600 : 400, color: isActive ? 'var(--text-primary)' : isHovered && canClick ? 'rgba(240,240,240,0.9)' : 'var(--text-secondary)', transition: 'color 0.15s ease' }}>{nav.label}</span>
                  </div>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--text-muted)', paddingLeft: 24 }}>{nav.sub}</span>
                </div>
              </div>
            )
          })}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)', overflow: 'hidden' }}>
              ...
              {activeImg ? (
                <>
                  <div style={{ alignSelf: 'flex-start', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--accent-blue)', background: 'rgba(99,179,237,0.1)', border: '1px solid rgba(99,179,237,0.25)', borderRadius: 4, padding: '3px 8px', letterSpacing: '0.08em' }}>step {activeStepNum} / 5</span>
                    <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, fontWeight: 500, color: 'var(--text-primary)' }}>{STEPS.find(s => s.id === activeSection)?.label}</span>
                  </div>
                ...
                <div style={{ margin: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, color: 'var(--text-muted)' }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11 }}>no output for this step yet</span>
                </div>
              )}
            </div>
          )}

          {/* VIEW: GCP Table Split Layout */}
          {activeSection === 'gcp-table' && result && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'row', overflow: 'hidden' }}>
                      ...
                      {result.gcpData.map((gcp) => {
                        ...
                        return (
                          <tr
                            ...
                            style={{
                              cursor: 'pointer', background: isSelected ? 'rgba(183, 148, 244, 0.15)' : 'transparent',
                              borderBottom: '1px solid rgba(255,255,255,0.03)', color: isSelected ? 'white' : 'var(--text-secondary)', transition: 'all 0.1s ease'
                            }}
              ...
              {/* Right Split: Free Flow Spatial Verifier */}
              <div style={{ width: '45%', display: 'flex', flexDirection: 'column', background: '#030407' }}>
                <div style={{ padding: '8px 16px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'center', zIndex: 10 }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, fontWeight: 500, marginRight: 'auto' }}>Spatial Verifier</span>
                ...
                {/* INTERACTIVE FREE FLOW CONTAINER */}
                <div
                  ref={containerRef}
                  style={{
                    flex: 1,
                ...
                >
                  ...
                  <div style={{
                    ...
                    {/* The 1:1 Aspect Ratio Canvas (640x640 proxy map logic) */}
                    <div style={{ position: 'relative', width: '100%', maxWidth: '640px', aspectRatio: '1/1' }}>

                      {/* draggable=false stops the browser from picking up the image file while panning */}
                      <img
                        src={stepImages[5]}
                        draggable={false}
                      ...
                      {/* High precision SVG absolute overlay. Divides radiuses by scale to keep them optically uniform on zoom! */}
                      <svg viewBox="0 0 640 640" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
                        {result.gcpData.map(gcp => {
                          const isSelected = selectedGcpId === gcp.id
                          return (
                            <g key={`gcp-${gcp.id}`}>
                              {/* Distinct Track Point */}
                              <circle
                              ...
                              {/* Selected Visuals: Red Line and Predicted mathematical location */}
                              {isSelected && (
                                <>
                                  <line
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, width: '100%', maxWidth: 600 }}>
                <div className="card" style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', textAlign: 'center', background: 'var(--bg-secondary)' }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, color: 'var(--text-muted)' }}>Root Mean Square Error (RMSE)</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 36, fontWeight: 600, color: 'var(--accent-purple)' }}>{rmse.toFixed(3)}<span style={{ fontSize: 14, color: 'var(--text-muted)' }}> px</span></span>
                </div>

                <div className="card" style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', textAlign: 'center', background: 'var(--bg-secondary)' }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, color: 'var(--text-muted)' }}>Mean Absolute Error (MAE)</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 36, fontWeight: 600, color: 'var(--accent-blue)' }}>{mae.toFixed(3)}<span style={{ fontSize: 14, color: 'var(--text-muted)' }}> px</span></span>
                </div>

                <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center', textAlign: 'center' }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, color: 'var(--text-muted)' }}>Maximum Outlier</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 24, fontWeight: 500, color: 'var(--accent-red)' }}>{maxErr.toFixed(3)}<span style={{ fontSize: 12 }}> px</span></span>
                </div>

                <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center', textAlign: 'center' }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 12, color: 'var(--text-muted)' }}>Minimum Inlier</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 24, fontWeight: 500, color: 'var(--accent-green)' }}>{minErr.toFixed(3)}<span style={{ fontSize: 12 }}> px</span></span>
                </div>
              </div>
            </div>
          )}
